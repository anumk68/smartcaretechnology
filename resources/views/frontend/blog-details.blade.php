                ...
                <div class="row g-4">
                    <div class="col-lg-8 order-2 order-lg-1">
                            ...
                            <div class="blog__content p-0">
                                <ul class="pb-3 pt-30 bor-bottom d-flex gap-4 flex-wrap align-items-center">
                                    ...
                                    <li>
                                        ...
                                        <a href="{{ url('blogs/' . $blog->category->slug) }}"><span
                                                class="primary-hover transition">{{ $blog->category->category_name }}</span></a>
                                    </li>
                                </ul>
                        ...
                        <div class="blog-single__review mt-60">
                            ...
                            <h3>{{ $blog_review->count() }} Reviews</h3>

                            <div id="review-list">
                                @foreach ($blog_review as $index => $blogReview)
                                    <div
                                        class="blog-single__comment d-flex gap-4 mt-60 pb-60 bor-bottom review-item {{ $index >= 2 ? 'd-none extra-review' : '' }}">
                                        <div class="image">
                                            <img src="{{ asset('public/' . $blogReview->blog->banner) }}" alt="image">
                                        </div>
                                        <div class="content">
                                            <div
                                                class="head d-flex flex-wrap gap-2 align-items-center justify-content-between">
                                                <div class="con">
                                                    <h5><a href="#0" class="primary-hover">{{ $blogReview->name }}</a>
                                                    </h5>
                                                    <span>{{ $blogReview->created_at->format('F d, Y \a\t g:i a') }}</span>
                                                </div>
                                                <div class="star text-warning review-star">
                                                    @for ($i = 1; $i <= 5; $i++)
                                                        @if ($i <= $blogReview->rating)
                                            ...
                                            <p class="mt-30 mb-4">{{ $blogReview->comment }}</p>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
                            ...
                            <div class="contact__form mt-60">
                                ...
                                <form action="{{ route('blog.review.store') }}" method="post" autocomplete="off">
                                    @csrf
                                    ...
                                    <div class="row">
                                        <div class="col-6">
                                            ...
                                            <input id="name" class="bg-transparent bor" name="name"
                                    ...
                                    <div class="text-area">
                                        ...
                                        <textarea class="bg-transparent bor" id="comment" name="comment" placeholder="Write Comment">{{ old('comment') }}</textarea>
                                        @error('comment')
                                            <small class="text-danger">{{ $message }}</small>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="btn-two mt-3">
                                        ...
                                        <button class="btn-one" type="submit">
                                            Post Comment <i class="fa-regular fa-arrow-right-long"></i>
                                        </button>
                                    </div>
                                </form>
                        ...
                        <div class="blog-single__right-item">
                            <div class="item sub-bg mb-30">
                                ...
                                <div class="serach-bar">
                                    <input type="text" id="categorySearch" placeholder="Search Blog"
                                        autocomplete="off">
                                    <button type="button"><i class="fa-regular fa-magnifying-glass"></i></button>
                                    <div id="searchResults"
                                        class="search-dropdown bg-white border rounded mt-1 position-absolute w-100"
                            ...
                            <div class="item sub-bg mb-30">
                                ...
                                <ul class="category">
                                    @foreach ($blogCategory as $Bcategory)
                                        <li><a
                                                href="{{ url('blogs/' . $Bcategory->slug) }}">{{ $Bcategory->category_name }}</a>
                                            <span>({{ $Bcategory->blog_count }})</span>
                                        </li>
                                    @endforeach
                            ...
                            <div class="item sub-bg mb-30">
                                ...
                                <ul class="single-post">
                                    @foreach ($resentBlogs as $Bresent)
                                        <li>
                                            <img src="{{ asset('public/' . $Bresent->banner) }}" alt="image"
                                                height="78" width="79">
                                            <div class="con">
                                                <span><svg class="me-1" width="20" height="20"
                                                        ...
                                                        <path
                                                            ...
                                                            fill="#3C72FC" />
                                                    </svg>{{ $Bresent->created_at->format('d M, Y') }}</span>
                                                <h5 class="mt-2"><a href="{{ url('blog/' . $Bresent->slug) }}"
                                                        class="primary-hover">{{ $Bresent->title }}</a></h5>
                                            </div>
                                        </li>
                                    @endforeach
                                </ul>
        ...
        $(document).ready(function() {
            $('#categorySearch').on('keyup', function() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (data.length > 0) {
                            data.forEach(function(blog) {
                                html += `<a href="{{ url('/blog') }}/${blog.slug}" class="d-block px-3 py-2 border-bottom text-dark text-decoration-none">
                                    ${blog.title}
                                 </a>`;
                            });
                        } else {
                            html = '<div class="px-3 py-2">No blogs found</div>';
                        }
                    ...
                    error: function() {
                        $('#searchResults').html(
                            '<div class="px-3 py-2">Something went wrong.</div>').show();
                    }
                });
            });
    ...
    <script>
        $(document).ready(function() {
            $('#toggle-reviews-btn').click(function() {
                let $extraReviews = $('.extra-review');

                if ($extraReviews.hasClass('d-none')) {
                    $extraReviews.removeClass('d-none');
                    $(this).text('See Less');
                } else {
                    $extraReviews.addClass('d-none');
                    $(this).text('See More');
                    $('html, body').animate({
